...
- `Policy-LearnedFittedQ-100-250ms`
...
`Policy-LearnedFittedQ-100-250ms` uses an offline fitted-Q style loop over the same discrete action set. It is trained from logged trajectories generated by burst-aware, LinUCB, TinyMLP, offline contextual, and random behavior policies on training seeds `[181, 191, 193, 197, 199, 211]`, then evaluated on held-out seeds `[223, 227, 229, 233]`.

The held-out regime family currently contains:
...
Published held-out outputs are written to:

- `docs/benchmarks/simulator_heldout_policy_profile.json`
- `docs/benchmarks/simulator_heldout_policy_profile.md`
- `docs/benchmarks/simulator_heldout_policy_profile.csv`
