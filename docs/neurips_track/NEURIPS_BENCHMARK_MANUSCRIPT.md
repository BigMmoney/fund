...
The results are organized around three layers: constraint-aware base tension, calibrated persistence, and counterfactual attribution. The point is not to enumerate many controller anecdotes. The point is to show that the same latency-welfare conflict survives these three evidence layers.
...
This paper makes one central claim: infrastructure optimization can systematically conflict with retail welfare under real-data-calibrated synthetic market constraints. Across base runs, calibrated reruns, and counterfactual controls, controllers that optimize aggressively for latency and fill throughput tend to widen surplus-transfer gap, while value-style learners remain more stable on the latency-welfare frontier than online learners that drift toward latency-favoring operating points. The benchmark is therefore useful not just for mechanism comparison, but for studying learning under infrastructure constraints where settlement correctness, mechanism choice, and retail outcome are evaluated jointly.
