...
\title{Beyond Matching-Only Evaluation: Ledger-Aware Benchmarking for Learning in Market Infrastructure}
...
\begin{abstract}
Existing market-learning evaluations usually emphasize matching outcomes, latency, fills, or PnL-style summaries, and can therefore mis-rank controllers that would be unacceptable once settlement admissibility and welfare-sensitive outcomes are included. We introduce a ledger-aware benchmark that combines seedable agent-based order flow, mechanism variation, explicit conservation/non-negativity/replay invariants, a shared learning API, and a compact retail-welfare decomposition. The main empirical result is an evaluation failure: controller rankings and operating points change materially once settlement-aware and welfare-aware evaluation is used. Under the full protocol, controllers lie on a stable latency--welfare frontier: methods that improve tail latency and fill throughput often widen surplus-transfer gap, while more balanced methods recover retail-sensitive outcomes only by paying some latency cost. Value-style learners remain more stable on that frontier than online learners, which drift toward latency-favoring operating points unless checkpoint selection is treated as part of the protocol. These patterns persist under held-out regimes, reward perturbations, stress sweeps, and a first-pass Binance spot calibration envelope, suggesting that learning in market infrastructure requires settlement-aware evaluation rather than matching-only benchmarks.
\end{abstract}

\section{Introduction}
Electronic markets are shaped jointly by matching rules, latency structure, and settlement semantics. Yet many market-learning evaluations still study matching in isolation and summarize controller quality with latency, fills, or PnL-style proxies alone. For machine learning, that is not just an omission of detail. It creates an evaluation problem: a controller that looks strong in a matching-only simulator can still be misleading or unacceptable once settlement admissibility and welfare-sensitive outcomes are part of the protocol.

This paper develops a benchmark-oriented layer on top of an existing ledger-first market-infrastructure prototype. The benchmark is designed around a narrower but more ML-facing question than the original systems line: when mechanism choice, learned control, settlement invariants, and retail-sensitive measurement are placed in the same loop, do the conclusions about which controllers are good remain the same? We answer that question with a seedable environment that combines explicit conservation/non-negativity/replay checks, immediate and batched execution regimes, a shared adapter API, and a compact welfare decomposition.

The main result is an evaluation-failure story rather than a new RL update rule. Under the full protocol, controllers lie on a stable latency--welfare frontier: aggressive latency optimization improves p99 and fill throughput but tends to widen surplus-transfer gap, while more balanced controllers recover retail-sensitive outcomes only by paying some latency cost. Under the calibrated protocol, value-style learners remain more stable on that frontier than online learners, which drift toward latency-favoring operating points unless checkpoint selection is treated as part of the protocol.

Counterfactual controls make the necessity claim sharper. In the new ranking-shift artifact, moving from the full calibrated protocol to \texttt{matching\_only} changes all four policy ranks and yields Kendall $\tau=-0.67$, whereas \texttt{no\_settlement} leaves the same ranking unchanged with $\tau=1.0$. Removing welfare terms from the reward also changes three of four ranks ($\tau=0.33$). The point is therefore not that settlement dominates every aggregate metric. The point is that admissibility, welfare-sensitive outcomes, and protocol choice determine which conclusions can be trusted.
...
\begin{enumerate}[leftmargin=*]
  \item It defines a ledger-aware benchmark in which mechanism experiments, learned controllers, deterministic settlement checks, and welfare-sensitive outcomes are evaluated under one shared protocol.
  \item It shows that simplified evaluation protocols can materially change controller rankings: \texttt{matching\_only} reverses the calibrated policy ordering, while the full protocol and \texttt{no\_settlement} agree on the same ranking.
  \item It reports a unified learning story across offline and online controllers, including held-out regime evaluation, learning-curve dynamics, reward sensitivity, checkpoint-selection effects, and a standardized calibrated leaderboard in which value-style learners are more stable than online policy updates.
  \item It strengthens the measurement and realism story with a four-dimensional stress surface, welfare-robustness summaries, and a first closed-loop market-data calibration pass in which the same latency--welfare tension persists.
\end{enumerate}
...
\section{Results}
The results are organized around four layers: the latency--welfare frontier itself, learning dynamics, protocol effects, and robustness under calibration and stress. The main claim is not only that a latency--welfare conflict exists. It is that simplified evaluation protocols can hide or distort that conflict, while the full protocol exposes stable differences between controller families.
...
\addplot[mark=*, thick] coordinates {(1,1) (2,3) (3,1) (4,1)};
\addplot[mark=square*, thick] coordinates {(1,2) (2,4) (3,2) (4,3)};
\addplot[mark=triangle*, thick] coordinates {(1,3) (2,2) (3,3) (4,4)};
\addplot[mark=diamond*, thick] coordinates {(1,4) (2,1) (3,4) (4,2)};
\legend{\texttt{iql},\texttt{fitted\_q},\texttt{burst\_aware},\texttt{ppo\_clip}}
\end{axis}
\end{tikzpicture}
...
Figure~\ref{fig:protocolranks} is the most direct statement of benchmark necessity. The full protocol ranks \texttt{iql} first and \texttt{fitted\_q} second, but the \texttt{matching\_only} simplification reverses that ordering and promotes \texttt{ppo\_clip} to the top position. The protocol question is therefore not cosmetic. Changing the evaluation loop changes what appears to be a good controller.
...
The calibrated-learning protocol now reports explicit train / validation / held-out splits rather than only a single tuned rerun. On held-out calibrated regimes, \texttt{burst\_aware} reaches $47.16 \pm 3.43$ fills per second with welfare gap $5.8322 \pm 0.1675$, \texttt{fitted\_q} reaches $56.86 \pm 1.89$ fills per second with welfare gap $3.9602 \pm 0.2672$, \texttt{ppo\_clip} reaches $40.54 \pm 1.85$ fills per second with welfare gap $6.6590 \pm 0.3586$, and \texttt{iql} reaches $58.86 \pm 1.72$ fills per second with welfare gap $3.9602 \pm 0.2590$. The strongest calibrated held-out tradeoff therefore comes from the IQL-style baseline rather than the simple PPO-style controller. A paired sign-flip comparison on the calibrated held-out regimes shows that \texttt{iql} improves both welfare gap and p99 latency relative to \texttt{ppo\_clip} with exact $p=3.1 \times 10^{-5}$ under the current protocol. The standardized calibrated leaderboard in \texttt{simulator\_policy\_leaderboard} therefore ranks \texttt{iql} first and \texttt{fitted\_q} second under one fixed score combining fills, p99, retail surplus, adverse selection, and surplus-transfer gap. The stronger Double-DQN artifact then adds a second protocol finding: once online learning is strong enough, checkpoint selection itself becomes part of the benchmark because intermediate policies can dominate final ones on the latency-welfare frontier. Under the calibrated envelope, value-style learners are therefore more stable on the latency-welfare frontier, whereas online learners are more prone to drift toward latency-favoring operating points unless checkpoint selection is treated as part of the protocol.

The counterfactual-control artifact adds three negative controls around the calibrated benchmark. In \texttt{matching\_only}, p99 latency collapses to $1000$ ms but retail surplus stays negative and welfare gap remains large ($4.5772$ to $5.2132$ across published policies). More importantly, the policy ranking changes materially: relative to the full control protocol, all four policy ranks move and Kendall $\tau=-0.67$. In \texttt{no\_settlement}, the mechanism and welfare measurements remain unchanged while settlement application is removed, and the same policy ranking is preserved with Kendall $\tau=1.0$. In \texttt{no\_welfare\_reward}, the PPO-style controller improves to $54.49 \pm 4.34$ fills per second and $3312.50 \pm 227.12$ ms p99, but still leaves a substantial welfare gap at $3.3874 \pm 0.1272$; the protocol ranking also changes, with three of four policy ranks moving and Kendall $\tau=0.33$. These controls sharpen the benchmark claim: the main tension does not disappear when reward weights are altered, and controller conclusions can change qualitatively when the infrastructure setting is reduced to matching-only execution. The \texttt{no\_settlement} control should not be read as evidence that settlement is unimportant. It should be read more narrowly: in the present aggregate setup, mechanism-level summary statistics are dominated by the matching/controller layer, while settlement still defines correctness, admissibility, and the trust boundary under which a learned controller would be acceptable in infrastructure use.
...
\midrule
\texttt{matching\_only} & -0.67 & 4 / 4 & 1 \\
\texttt{no\_settlement} & 1.00 & 0 / 4 & 2 \\
\texttt{no\_welfare\_reward} & 0.33 & 3 / 4 & 1 \\
\bottomrule
\end{tabular}
